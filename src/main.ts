import { parseContainer } from "./graphing/parser";
import "./style.css";

const calc = document.getElementById("calculator");
const { Desmos } = window as any;

const calculator = Desmos.GraphingCalculator(calc, { autosize: false });
...
for (let c = 0; c < containers.length; c++) {
  ...
  for (let i = 0; i < container.expressions.length; i++) {
    let next = container.next();
    let parsed = container.parse(next);
